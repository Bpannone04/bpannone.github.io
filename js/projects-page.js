...
class ProjectsPage {
    ...
    renderProjectsSection() {
        const projectCards = projects
            ...
            const tags = project.tags
                .map((tag) => `<span class="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-sm">${tag}</span>`)
                .join("");
    ...
    initProjectDetailListeners() {
        document.querySelectorAll(".project-card").forEach((card) => {
            const openDetail = (e) => {
                if (e.target.closest("[data-no-card-open]"))
                    return;
                const idx = e.currentTarget.getAttribute("data-project-index");
                if (idx !== null)
                    this.openProjectDetailModal(parseInt(idx, 10));
            };
